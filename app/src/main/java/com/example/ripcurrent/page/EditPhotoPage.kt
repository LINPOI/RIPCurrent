...
fun EditPhotoPage(modifier: Modifier, navController: NavHostController) {
    ...
    Scaffold(modifier = modifier.fillMaxSize(),
        ...
        bottomBar = {
            Column()
            {
                if(setwidth)Slider(value = strokeWidth, onValueChange ={ strokeWidth = it }, steps = 10, valueRange = 10f..100f)
                Row(
                    modifier
                        .fillMaxWidth()
                        .background(MaterialTheme.colorScheme.primary)
        ...
        sketchbookController.setPaintStrokeWidth(strokeWidth)
        //畫布
        Sketchbook(
            modifier = Modifier
                .fillMaxSize()
